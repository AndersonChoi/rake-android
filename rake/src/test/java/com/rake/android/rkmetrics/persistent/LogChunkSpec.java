...
public class LogChunkSpec {

    @Test
    public void create_lastId_또는_logs_가_null_또는_logs_size_가_0_일때() {

        assertThat(LogChunk.create(null, null)).isNull();
        assertThat(LogChunk.create(null, Collections.EMPTY_LIST)).isNull();
        assertThat(LogChunk.create("lastId-1", Collections.EMPTY_LIST)).isNull();
    }
    ...
    public void create__token_과_url_이_여러개일때() {
        ...
        List<LogChunk> chunks = LogChunk.create("lastId", logs);
    ...
    public void default_생성자를_리플렉션으로_직접호출시()
        ...
        Class clazz = Class.forName(LogChunk.class.getName());
        clazz.newInstance();
    }
}
